...
func ParseAndStubFromFile(interfaceNames []string, filePath string, formatted bool) ([]byte, error) {
	...
	overlay, err := mimicGeneratedOverlay(filePath)
	if err != nil {
		return nil, fmt.Errorf("could not build overlay: %w", err)
	}
...
// mimicGeneratedOverlay returns an overlay that replaces any existing
// mimic-generated *_fake.go files in the same directory with a minimal
// package declaration. This prevents stale compile-time interface assertions
// from blocking packages.Load when regenerating fakes after an interface change.
func mimicGeneratedOverlay(sourceFilePath string) (map[string][]byte, error) {
	dir := filepath.Dir(sourceFilePath)

	fset := token.NewFileSet()
	f, err := parser.ParseFile(fset, sourceFilePath, nil, parser.PackageClauseOnly)
	if err != nil {
		return nil, fmt.Errorf("could not parse package name from %s: %w", sourceFilePath, err)
	}
	pkgDecl := []byte("package " + f.Name.Name + "\n")
	...
	for _, entry := range entries {
		if entry.IsDir() || !strings.HasSuffix(entry.Name(), "_fake.go") {
			continue
		}
		...
		if !bytes.HasPrefix(content, []byte("// Code generated by mimic")) {
			continue
		}
		if overlay == nil {
			overlay = make(map[string][]byte)
		}
		overlay[fullPath] = pkgDecl
	}
	return overlay, nil
}
